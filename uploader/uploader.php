...
// require_once($CFG->dirroot . '/local/qmapi/uploader/upload_system.php');
// require_once($CFG->dirroot . '/local/qmapi/uploader/Uploader.php');
...
$context =  context_user::instance($_REQUEST['userid']);

// $fs = get_upload_file_storage();
$fs = get_file_storage();
$totalsize = 0;
$files = array();
...
foreach ($files as $file) {
    ...
    $file_record->component = $_REQUEST['component'];
    $file_record->contextid = $_REQUEST['contextid'];
    $file_record->userid    = $_REQUEST['userid'];
    $file_record->filearea   = $_REQUEST['filearea'];
    $file_record->filename   = $file->filename;
    $file_record->filepath   = $_REQUEST['filepath'];
    $file_record->itemid    = $_REQUEST['itemid'];
    $file_record->license   = $_REQUEST['license'];
    $file_record->author    = $_REQUEST['author'];
    ...
    if ($stored_file->errorcode == 'storedfilenotcreated') {
        ...
        $results =  [
            ...
            'statusCode' => 406,
            'file' => [
                'contextid' => $stored_file->a->contextid,
                'filename'   => $stored_file->a->filename,
                'filepath'   => $stored_file->a->filepath
            ],
        ];
