...
class TrueFalseQuestion
{
    ...
    public function create_editing_form(
        $submiturl,
        $question,
        $category,
        $contexts,
     ...
     * This function in turn calls {@link save_question_option}
     ...
     */
    public function save_question_true_false($question, $clientForm)
    {
        ...
        if (empty($clientForm[0]['generalfeedback'])) {
            ...
            $question->generalfeedback = trim($clientForm[0]['generalfeedback']);
        }
        $question->generalfeedbackformat = 1;

        if ($question->name === '') {
            $question->name = shorten_text(strip_tags($clientForm[0]['name']), 15);
            if ($question->name === '') {
                $question->name = '-';
            }
        }
        ...
        if (isset($clientForm[0]['defaultmark'])) {
    ...
    /**
     * Can be used to {@link save_question_option()} to transfer the combined
     ...
     */
    protected function save_combined_feedback_helper(
        $options,
        $formdata,
        $context,
        $withparts = false
     ...
     */
    protected function initialise_question_answers(
        object $question,
        array $answers,
     ...
     */
    public function get_context_by_category_id($category)
    {
        global $DB;
     ...
     */
    protected function move_files_in_answers(
        $questionid,
        $oldcontextid,
        $newcontextid,
        $answerstoo = false
     ...
     *      as well as an 'answerfeedback' one. Default false.
     */
    protected function move_files_in_combined_feedback(
        $questionid,
        $oldcontextid,
        $newcontextid
    ) {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class question_possible_response_api
{
    /**
